...
Player::Player(SDL_Renderer* renderer, double speed)
    : mSpeed(speed), mPosX(SCREEN_WIDTH / 2), mPosY(SCREEN_HEIGHT / 2),
      mVelX(0), mVelY(0), mRadius(30),
      mMoveUp(false), mMoveDown(false), mMoveLeft(false), mMoveRight(false) {}
...
void Player::update(double deltaTime) {
    mVelX = (mMoveRight - mMoveLeft) * mSpeed;
    mVelY = (mMoveDown - mMoveUp) * mSpeed;

    mPosX += mVelX * deltaTime;
    mPosY += mVelY * deltaTime;

    mPosX = clamp(mPosX, static_cast<double>(mRadius), static_cast<double>(SCREEN_WIDTH - mRadius));
    mPosY = clamp(mPosY, static_cast<double>(mRadius), static_cast<double>(SCREEN_HEIGHT - mRadius));
...
void Player::fireBullet(int mouseX, int mouseY) {
    ...
    // 新增子彈
    bullets.emplace_back(mPosX, mPosY, dirX + mVelX / bulletSpeed, dirY + mVelY / bulletSpeed, bulletSpeed);
}
